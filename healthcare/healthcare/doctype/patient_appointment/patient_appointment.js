...
frappe.ui.form.on("Patient Appointment", {
	...
	onload: function (frm) {
		if (frm.is_new()) {
			...
			frm.disable_save();
			if (frappe.route_options && frappe.route_options.show_availability) {
				setTimeout(() => frappe.ui.healthcare_appointment_dialog({frm: frm}), 500);
			}
		}
	},
		...
		if (
			...
			frm.add_custom_button(__("Reschedule"), function () {
				if (frm.doc.appointment_for == "Practitioner") {
					frappe.ui.healthcare_appointment_dialog({frm: frm});
				} else {
